...
export default {
  ...
  computed: {
    selectedBanner() {
      return this.banners.find((banner) => banner.name === this.selectedBannerName)
    },
  },
  methods: {
...
<template>
  <div class="d-flex flex-column gap-3">
    <div class="p-3 rounded app-surface d-flex align-items-baseline justify-content-between">
      ...
      <span v-if="pinnedBannerName" class="small text-body-secondary"
        >Pinned: {{ pinnedBannerName }}</span
      >
    </div>
